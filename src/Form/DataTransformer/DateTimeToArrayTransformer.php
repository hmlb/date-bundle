...
class DateTimeToArrayTransformer extends BaseDateTimeTransformer
{
    ...
    public function reverseTransform($value)
    {
        ...
        if (!empty($value['month']) && !empty($value['day']) && !empty($value['year']) && false === checkdate(
                $value['month'],
                $value['day'],
                $value['year']
            )
        ...
        try {
            $date = Date::create(
                empty($value['year']) ? 1970 : (int) $value['year'],
                empty($value['month']) ? 1 : (int) $value['month'],
                empty($value['day']) ? 1 : (int) $value['day'],
                empty($value['hour']) ? 0 : (int) $value['hour'],
                empty($value['minute']) ? 0 : (int) $value['minute'],
                empty($value['second']) ? 0 : (int) $value['second'],
